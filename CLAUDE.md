...
- `adapters/` — 10 source-language adapters via `ast-grep-core` + `ast-grep-language`. `LanguageAdapter` trait in `base.rs`. Sources: rust / csharp / java / kotlin / scala / typescript / python / go / markdown. Dispatcher: `parse_file(path) -> Option<ParseResult>`.
